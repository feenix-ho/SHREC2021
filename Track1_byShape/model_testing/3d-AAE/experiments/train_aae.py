...
def main(config):
    ...
    log.debug('Device variable: {device}')
    if device.type == 'cuda':
        log.debug('Current CUDA device: {torch.cuda.current_device()}')
    ...
    if dataset_name == 'shapenet':
        ...
        raise ValueError('Invalid dataset name. Expected `shapenet` or '
                         '`faust`. Got: `{dataset_name}`')
    ...
    if config['reconstruction_loss'].lower() == 'chamfer':
        ...
        raise ValueError('Invalid reconstruction loss. Accepted `chamfer` or '
                         '`earth_mover`, got: {config["reconstruction_loss"]}')
    #
    # Float Tensors
    #
    ...
    if starting_epoch > 1:
        G.load_state_dict(torch.load(
            join(weights_path, '{starting_epoch-1:05}_G.pth')))
        E.load_state_dict(torch.load(
            join(weights_path, '{starting_epoch-1:05}_E.pth')))
        D.load_state_dict(torch.load(
            join(weights_path, '{starting_epoch-1:05}_D.pth')))

        D_optim.load_state_dict(torch.load(
            join(weights_path, '{starting_epoch-1:05}_Do.pth')))

        EG_optim.load_state_dict(torch.load(
            join(weights_path, '{starting_epoch-1:05}_EGo.pth')))
    ...
    for epoch in range(starting_epoch, config['max_epochs'] + 1):
        ...
        for i, point_data in enumerate(points_dataloader, 1):
            ...
            log.debug('[{epoch}: ({i})] '
                      ...
                      '(GP: {loss_gp.item(): .4f}) '
                      'Loss_EG: {loss_eg.item():.4f} '
                      '(REC: {loss_e.item(): .4f}) '
                      'Time: {datetime.now() - start_epoch_time}')

        log.debug(
            '[{epoch}/{config["max_epochs"]}] '
            'Loss_D: {total_loss_d / i:.4f} '
            'Loss_EG: {total_loss_eg / i:.4f} '
            'Time: {datetime.now() - start_epoch_time}'
        )
        ...
        for k in range(5):
            fig = plot_3d_point_cloud(X[k][0], X[k][1], X[k][2],
                                      ...
                                      title=str(epoch))
            fig.savefig(
                join(results_dir, 'samples', '{epoch:05}_{k}_real.png'))
            plt.close(fig)
        ...
        for k in range(5):
            fig = plot_3d_point_cloud(fake[k][0], fake[k][1], fake[k][2],
                                      ...
                                      title=str(epoch))
            fig.savefig(
                join(results_dir, 'samples', '{epoch:05}_{k}_fixed.png'))
            plt.close(fig)
        ...
        for k in range(5):
            ...
            fig.savefig(join(results_dir, 'samples',
                             '{epoch:05}_{k}_reconstructed.png'))
            plt.close(fig)

        if epoch % config['save_frequency'] == 0:
            torch.save(G.state_dict(), join(weights_path, '{epoch:05}_G.pth'))
            torch.save(D.state_dict(), join(weights_path, '{epoch:05}_D.pth'))
            torch.save(E.state_dict(), join(weights_path, '{epoch:05}_E.pth'))

            torch.save(EG_optim.state_dict(),
                       join(weights_path, '{epoch:05}_EGo.pth'))

            torch.save(D_optim.state_dict(),
                       join(weights_path, '{epoch:05}_Do.pth'))
